package homeWork.javaPro.hw01_03052023;

import java.util.*;
import java.util.stream.Collectors;

public class JavaStreamTask1 {
    public static void main(String[] args) {
        ...
        int count = 1;
        List<Match> collect = matches.stream()
                .filter(match -> match.getDate().get(Calendar.DATE) <= 15)
                .collect(Collectors.toList());
        for (Match match : collect){
        ...
        System.out.println("Количество матчей: " + matches.stream().count());
        System.out.println("-----------");

        // выбрать топ 10 самых посещаемых матчей
        count = 1;
        List<Match> top10Match = matches.stream()
                .sorted(Comparator.comparing(Match::getWatchers).reversed())
                .limit(10)
                .toList();
        for (Match match : top10Match){
            System.out.println(count+". "+match);
            count++;
        }
        System.out.println("-----------");
        ...
        count=1;
        List<Match> wwkArena = matches.stream()
                .filter(match -> match.getStadium().equals("WWK Arena"))
                .sorted((m1,m2)->Math.abs(m2.getResult1()-m2.getResult2())-Math.abs(m1.getResult1()-m1.getResult2()))
                .limit(3)
                .toList();
        for (Match match : wwkArena){
            System.out.println(count+". "+match);
            count++;
        }
        System.out.println("-----------");

        // посчитать сколько матчей содержат упоминания о стадионе WWK Arena
        count=1;
        List<Match> wwkArena1 = matches.stream().filter(match -> match.getStadium().equals("WWK Arena")).toList();
        for (Match match : wwkArena1){
            System.out.println(count+". "+match);
            count++;
        }
        System.out.println("-----------");
        ...
        // без использования Stream и посчитать время работы метода
        countGuestMatches(matches);
        
        // и с помощью stream API и посчитать время работы метода
        countGuestMatchesStream(matches);
    ...
    public static Map<String, Integer> countGuestMatches(List<Match> matches) {
        ...
        Map<String, Integer> guestMatchesCount = new HashMap<>();
        for (Match match : matches) {
            String team = match.getTeam2();
            int matchesCount = guestMatchesCount.getOrDefault(team, 0);
            guestMatchesCount.put(team, matchesCount + 1);
        }
        printMap(guestMatchesCount);
        long endTime = System.currentTimeMillis(); // засекаем время окончания работы метода
        System.out.println("Метод выполнился " + (endTime - startTime) + " милиСекунд.");
        System.out.println("---------------");
        return guestMatchesCount;
    }

    public static Map<String, Integer> countGuestMatchesStream(List<Match> matches) {
        long startTime = System.currentTimeMillis(); // засекаем время начала работы метода
        Map<String, Integer> guestMatchesCount1 = matches.stream()
                .collect(Collectors.groupingBy(Match::getTeam2,Collectors.summingInt(a->1)));

        printMap(guestMatchesCount1);
        long endTime = System.currentTimeMillis(); // засекаем время окончания работы метода
        System.out.println("Метод выполнился " + (endTime - startTime) + " милиСекунд.");
        System.out.println("---------------");
        return guestMatchesCount1;
    }
    public static void printMap(Map<String, Integer> map){
        int count = 1;
        for (Map.Entry<String,Integer> entry : map.entrySet()){
            System.out.println(count+". На выезде: "+entry.getKey()+", к-во матчей: "+ entry.getValue());
            count++;
        }
    }
